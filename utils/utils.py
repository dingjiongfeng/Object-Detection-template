...
label_color_map = {k: distinct_colors[i] for i, k in enumerate(label_map.keys())} #plane #000000 
...
def create_data_lists(voc12_path='../data/VOC2012', output_folder='../data/VOC2012/json'):
    ...
    for split in ['train', 'val']:
        ...
        with open(os.path.join(voc12_path, 'ImageSets/Main/'+split+'.txt')) as f:
            ids = f.read().splitlines() #都是返回列表 .read().splitlines()每一行后面没有\n  readlines 每一行后面有\n 
        print('id finish')
        for id in ids:
